...
\chapter*{In Defense of my Hubris}
...
In the worst case, writing this is clarifying my own thoughts and understanding. And having an open source topology textbook might be valuable, even if it isn't as good as others. 
...
We can now visualize all of the closure axioms:
...
\item Draw the discrete closure operator on $\{1,2,3\}$.
\item Draw the inidiscrete closure operator on $\{1,2,3\}$.
\item Draw a closure operator on $\{1,2,3\}$ that isn't the discrete or indiscrete closure operator.
\item Draw the finite-closed closure operator on $ℤ$. (Obviously, since $ℤ$ is infinite, you can't draw the entire thing. Draw a represantitive part and imagine what the entire thing would be like...)
